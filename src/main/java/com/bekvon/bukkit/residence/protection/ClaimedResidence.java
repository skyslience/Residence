...
//	}
    ...
    public boolean isSmallerThanMax(Player player, CuboidArea area, boolean resadmin) {
        ...
        ResidencePlayer rPlayer = plugin.getPlayerManager().getResidencePlayer(player);
        PermissionGroup group = rPlayer.getGroup();
        if (area.getXSize() > (group.getMaxX()+getPlayerPermissSize(player.getName()))+ IsLandAddAPI.getPlayerLandSize(player)) {
            plugin.msg(player, lm.Area_ToBigX, area.getXSize(), group.getMaxX());
            return false;
        }
        if (area.getYSize() > (group.getMaxY()+getPlayerPermissSize(player.getName()))+ IsLandAddAPI.getPlayerLandSize(player)) {
            plugin.msg(player, lm.Area_ToBigY, area.getYSize(), group.getMaxY());
            return false;
        }
        if (area.getZSize() > (group.getMaxZ()+getPlayerPermissSize(player.getName()))+ IsLandAddAPI.getPlayerLandSize(player)) {
    ...
    public int getPlayerPermissSize(String playerName){
        ...
        if(player != null){
            for(PermissionAttachmentInfo perms : player.getEffectivePermissions()){
                if (perms.getPermission().startsWith("residence.addsize.")) {
                    if (perms.getPermission().contains("residence.addsize.*")) {
                        size = 0;
                        break;
                    } else {
                        // Get the max value should there be more than one
                        String[] spl = perms.getPermission().split("residence.addsize.");
                        if (spl.length > 1) {
